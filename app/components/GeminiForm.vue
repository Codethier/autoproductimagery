  ...
  <UCard class="max-w-2xl mx-auto my-6">
    <template #header>
      <div class="flex items-center justify-between">
        ...
        <UBadge v-if="model" color="gray" variant="subtle">{{ model }}</UBadge>
      </div>
    </template>

    <form @submit.prevent="onSubmit" class="space-y-4">
      <UFormGroup label="Prompt" :error="errors.prompt">
        <UTextarea v-model="form.prompt" :rows="4" placeholder="Describe what you want Gemini to generate..." />
      </UFormGroup>

      <UFormGroup label="Attach images (optional)" :error="errors.images">
        ...
        <div v-if="previews.length" class="mt-3 grid grid-cols-3 gap-2">
          <div v-for="(p, idx) in previews" :key="idx" class="border rounded p-1 flex flex-col items-center">
            ...
            <UButton color="red" variant="soft" size="2xs" class="mt-1" @click="removeImage(idx)">Remove</UButton>
          </div>
        </div>
      </UFormGroup>

      <div class="flex gap-2 items-end">
        <UFormGroup label="Model (optional)" class="flex-1">
      ...
      <div v-if="result && (result.images?.length || 0) > 0" class="space-y-2">
        <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
          <div v-for="(img, i) in result.images" :key="i" class="flex flex-col items-start gap-2">
            ...
            <UButton color="gray" variant="soft" size="xs" @click="downloadImage(i)">Download</UButton>
          </div>
        </div>
      </div>
