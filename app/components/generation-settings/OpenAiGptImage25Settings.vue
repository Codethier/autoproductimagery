...
import {
  ...
  type OpenAiGptImage25Settings,
} from '~~/schemas/image-generation'

const settings = defineModel<OpenAiGptImage25Settings>({required: true})
  ...
  <div class="space-y-4">
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <label class="text-sm font-medium">Custom size
        ...
        <span v-if="sizeError" class="setting-error">This size does not meet GPT Image 2.5 constraints.</span>
      </label>
      <label class="text-sm font-medium">Quality
        <select v-model="settings.quality" class="setting-input">
          <option value="auto">Auto</option><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="xhigh">Extra high</option><option value="max">Max</option>
        </select>
    ...
    <div class="rounded-lg border border-gray-200 p-3 text-xs text-gray-600 dark:border-gray-700 dark:text-gray-300">
      Resolutions above 2560x1440 are experimental. Each selected product image gets its own generation job.
    </div>
  </div>
</template>
